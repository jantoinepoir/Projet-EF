...
def plot_mesh_2d(elemType, nodeTags, nodeCoords, elemTags, elemNodeTags,
    ...
    # On prend les 3 premiers nœuds (= triangle)
    # Pour des éléments d'ordre 2 (6 nœuds par triangle), seuls les 3 premiers nœuds sont les sommets géométriques. On les extrait pour construire la triangulation.    
    ...
    # Plot des noeuds des bords physiques
    for i, (name, dim) in enumerate(bnds):
        tags = bnds_tags[i]
        indices = tag_to_index[tags.astype(int)]
        # Pour chaque frontière physique, les nœuds correspondants sont affichés en couleur distincte afin de visualiser où s'appliquent les conditions limites
        ax.scatter(
            x[indices], y[indices],
            label=name,
            s=15,
...
def plot_fe_solution_2d_krogh(elemTags, elemNodeTags, nodeCoords, nodeTags, U, tag_to_dof,
    ...
    """
    Affiche la solution éléments finis 2D pour le modèle de Krogh
    ...
    # Si les coordonnées ne correspondent pas directement aux tags, on doit faire la correspondance manuellement
    ...
    for i, tag in enumerate(nodeTags):
        dof_idx = tag_to_dof[int(tag)]# Il convertit un tag Gmsh en indice de degré de liberté dans le vecteur solution U
...
def plot_fe_solution_2d_multicouche(elemTags, elemNodeTags, nodeCoords, nodeTags, U, tag_to_dof,
                        show_mesh=False, ax=None, cmap="hot", vmin=None, vmax=None, norm=None):
    """
    Affiche la solution éléments finis 2D pour le modèle multicouche.
    ...
    conn_reshaped = elemNodeTags.reshape(-1, nodes_per_elem)
    triangles = tag_to_dof[conn_reshaped[:, :3].astype(int)]
    # La connectivité (qui dit quels nœuds forment chaque triangle) est re-exprimée en indices de DDL plutôt qu'en tags Gmsh
 
    U = np.array(U).flatten()
 
    norm = None
    contour = ax.tricontourf(x, y, triangles, U, levels=100, cmap=cmap, norm=norm)
    #Trace le champ scalaire U en interpolant les valeurs nodales sur toute la surface triangulée, avec 100 niveaux de couleur
    if show_mesh:
        ax.triplot(x, y, triangles, color="white", linewidth=0.2, alpha=0.3)
